...
router.get("/summary", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'artist') {
      ...
      whereClause += ` AND (
        cr.artist_id IN (SELECT id FROM users WHERE label_id = $${paramCount}) OR
    ...
    const result = await client.query(`
      SELECT 
        ...
        COALESCE(COUNT(DISTINCT cr.artist_id), 0) as total_artists,
        ...
        COALESCE(SUM(CASE WHEN cr.finance_status = 'calculated' THEN cr.payable_amount ELSE 0 END), 0) as pending_payable,
        COALESCE(SUM(CASE WHEN cr.finance_status = 'paid' THEN cr.payable_amount ELSE 0 END), 0) as paid_amount
    ...
    try {
      const payoutResult = await client.query(`
        ...
        FROM payouts
        WHERE ${req.user.role === 'artist' ? 'owner_id = $1' : '1=1'}
